...
const CalculationResultModal: React.FC<CalculationResultModalProps> = ({ open, onClose, result, slipTitle, homemadeTotal, selectedRecipes, quantities, paidAmount }) => {
  ...
  const isCalorieCalculation = result?.type === 'calorie';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl p-0 w-full max-w-xl relative animate-fadeInUp border-2 border-violet-200 dark:border-violet-900/30">
        ...
        <div className="flex flex-col items-center pt-6">
          <h3 className="text-lg font-extrabold text-violet-700 dark:text-violet-400 text-center mb-1">{slipTitle || t('health.calculation_result')}</h3>
        </div>
        <div ref={slipCardRef} className="bg-gray-50 dark:bg-gray-800 rounded-xl p-5 shadow-inner mb-4 mx-4 mt-2">
          {/* Başlık */}
          ...
          <div className="text-center text-xs text-gray-500 dark:text-gray-400 mt-2">
            {t('health.powered_by')} {t('app.brand')} • {new Date().toLocaleTimeString('tr-TR')}
          </div>
        </div>
